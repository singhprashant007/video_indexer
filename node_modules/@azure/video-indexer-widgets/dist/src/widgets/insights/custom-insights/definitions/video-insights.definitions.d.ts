...
export type TopicReferenceType = 'VideoIndexer' | 'Wikipedia';
export interface IFramePattern extends IIndexElement {
    name: FramePatternType;
}
export type FramePatternType = 'RollingCredits' | 'Black';
...
export type SentimentType = 'Negative' | 'Neutral' | 'Positive' | 'Undetermined';
export interface IAudioEffect extends IIndexElement {
    type: AudioEffectType;
}
export type AudioEffectType = 'Unlabeled' | 'Baseline' | 'Laughter' | 'Clapping' | 'Speech' | 'Silence' | 'Gunshot' | 'Alarm' | 'DogBarking' | 'Crying' | 'CrowdReactions' | 'Explosion' | 'Screaming' | 'GlassShattering' | 'Siren';
export interface ISpeaker extends IIndexElement {
    name: string;
}
export type BrandType = 'None' | 'Transcript' | 'Ocr';
...
export type BrandReferenceType = 'Wiki';
export interface IIndexElementWithType<T> {
    id: number;
    instances: T[];
}
